...
void criarConta(){
    ...
    if(std::cin.fail()){
        std::cout << "Erro: numero da conta invalida." <<std::endl;
    ...
    if(encontrarConta(numero) !=nullptr){ //caso a conta ja exista ele vai encontrar uma posição no vector de contas 
        std::cout << "Erro:já Exsite uma conta com esse numero ." << std:: endl;
        limparBufferEntrada();
        return;
    }
    ...
    std::cout << "Digite o deposito inicial (0 se nao houver) :";
